...
class Linear_projection(nn.Module):
    ...
    def __init__(self, img_size=124, patch_size=3, in_chans=16, embed_dim=28):
        ...
        self.conv = nn.Conv2d(in_chans, int((in_chans - 3) / 2 + 1), kernel_size=self.kernel_size, stride=self.strides,
                              padding=1)
        self.relu = nn.ReLU(inplace=True)
        # self.measure_pos_embd = measure_pos_embd
        self.hybrid = hybrid_position_embedding()
        # self.proj = nn.Conv3d(1, embed_dim, kernel_size=(3, 1, 1), stride=(3, 1, 1))
        self.proj = nn.Conv3d(1, embed_dim, kernel_size=(2, 1, 1), stride=(2, 1, 1))
...
def Spectral_Mixer_Layer(dim, depth, kernel_size=(3, 1, 1)):
...
# def Spectral_Mixer_Layer(dim, depth, kernel_size=(2, 1, 1)):
#     return nn.Sequential(
#         *[nn.Sequential(
#             Residual(nn.Sequential(
#                 nn.Conv3d(dim, dim, kernel_size, groups=dim, padding=(int(kernel_size[0] / 2), 0, 0)),
...
def Spatial_Mixer_Layer(dim, depth, kernel_size=(1, 3, 3)):
    return nn.Sequential(
        *[nn.Sequential(
            Residual(nn.Sequential(
...
class Spectral_Spatial_Mixer_Block(nn.Module):
    ...
    def __init__(self, dim, depth, spec_kernel_size=(3, 1, 1), spa_kernel_size=(1, 3, 3)):
...
class HSI_Mixer_Net(nn.Module):
    ...
    def __init__(self, num_classes=112, img_size=64, patch_size=16, depth=1, in_chans=4, embed_dim=256):
        super(HSI_Mixer_Net, self).__init__()
        self.linear_projection = Linear_projection(img_size, patch_size, in_chans, embed_dim)
        self.spc_spa_mixer = Spectral_Spatial_Mixer_Block(dim=embed_dim, depth=depth, spec_kernel_size=(3, 1, 1),
                                                          spa_kernel_size=(1, 3, 3))
        # self.spc_spa_mixer = Spectral_Spatial_Mixer_Block(dim=embed_dim, depth=depth, spec_kernel_size=(2, 1, 1),
        #                                                   spa_kernel_size=(1, 3, 3))
...
#     x = torch.randn(128, 4, 64, 64)
    # input = torch.randn(1, 3, 224, 224)
    # net = HSI_Mixer_Net(num_classes = 112, img_size = 64, patch_size = 16, in_chans = 4, embed_dim=256)
# #     net = HSI_Mixer_Net(num_classes=16, img_size=input.size()[3], patch_size=3, in_chans=input.size()[1], embed_dim=28)
#     net.cuda()
    # out= net.descriptor(x.cuda())
#     summary(net, (16, 124, 124))
