# -*- coding: utf-8 -*-
""" Title: Convert HyTexila reflectance to radiance under extended D65 illuminant
...
### ------------------------------------------------------------------
illuminant_D65 = np.load('extended_D65_380_1000_interp1nm.npy')
_, idx_illu_for_hytexila = ST.find_closest_spectra_illuminant(HyTexila_centers,illuminant_D65[:,0])
selected_D65_values = illuminant_D65[idx_illu_for_hytexila,1]
selected_D65_values = selected_D65_values.astype(np.float32)
### ------------------------------------------------------------------
for i in tqdm(range(len(imageLists))):
    ...
    radiance_cube = ST.reflectance2radiance(cube, selected_D65_values)
