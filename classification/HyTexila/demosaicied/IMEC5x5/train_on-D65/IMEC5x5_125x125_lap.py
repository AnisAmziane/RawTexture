...
CONFIG = SimpleNamespace(images_path = '/home/anis/Documents/ms_base/others/HyTexila_bmvc/HyTexila_D65_IMEC25_demosaiced/',
                         model_name='LAP', input_channels=25, patch_size=(125,125), MSFA='Imec25', logits_size=112, device='cpu')
# -----------------------------------------
...
for j in tqdm(range(steps)):
        ...
        batch_features = executor(delayed(lbp.local_angles_pattern)(trainx[i], 8, 1, reference='avg', interpolate=False,
                                                   lbp_type='default') for i in range(len(data)))
        # free some memory
        del data,trainx[i * batch_size:(i + 1) * batch_size]
        gc.collect()
        batch_features = executor(delayed(lbp.np_histogram)(batch_features[i], 256, normalize=False) for i in range(len(batch_features)))
