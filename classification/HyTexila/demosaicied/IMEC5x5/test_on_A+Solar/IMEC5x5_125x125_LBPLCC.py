...
#*******************************************************
CONFIG = SimpleNamespace(images_path = '/home/anis/Documents/ms_base/others/HyTexila_bmvc/HyTexila_A_IMEC25_demosaiced/',
                         model_name='LBPLCC', input_channels=25, patch_size=(125,125), MSFA='Imec25', logits_size=112, device='cpu')
#
#-----------------------------------------------------------------------------------------#
...
list_test_features = []
batch_size = 500
steps = int(math.ceil(len(list_test_images) / batch_size))
i = 0
# memory efficient feature extraction
for j in tqdm(range(steps)):
    data = np.asarray(list_test_images[i * batch_size:(i + 1) * batch_size])
    list_train_test_ppi = executor(delayed(lbp.simple_lbp_ppi_hist)(data[i], 1) for i in range(len(data)))
    list_train_test_ppi = np.asarray(list_train_test_ppi)
    list_test_features_lcc = executor(
        delayed(lbp.lcc)(data[i], 8, 1, reference='avg', interpolate=False) for i in range(len(data)))
    list_test_features_lcc = executor(
        delayed(lbp.np_histogram)(list_test_features_lcc[i], 256, normalize=False) for i in
        range(len(list_test_features_lcc)))
    list_test_features_lcc = np.asarray(list_test_features_lcc)
    batch_features = np.concatenate((list_train_test_ppi, list_test_features_lcc), axis=1)
    # free some memory
    del data, list_test_images[i * batch_size:(i + 1) * batch_size], list_test_features_lcc, list_train_test_ppi
    gc.collect()
